...
### chatStore: Per-Step Context State

In-memory, double-keyed state feeding the step/subagent header cluster (see [rendering.md](rendering.md) § Step/Subagent Header Cluster):

| Field                 | Purpose |
| --------------------- | ------- |
| `stepContextFill`     | `sessionId → stepId → number` — the per-step context fill reported by step-scoped `context_fill` events. A subagent/executor step reports its **own** window fill, which the block header renders while the conductor's session-level fill (status bar) stays untouched — `handleContextFill` (useContextEvents) writes the step map and the session token sums, but not the session fill. |
| `stepContextTokens`   | `sessionId → stepId → Partial<StepContextTokens>` — the step's own context-window totals, carried by the same step-scoped `context_fill` events; feeds the header's optional "N of M" tooltip (the fill percentage alone never enables it). Entries may be partial (the first payload for a step may carry only one field); `setStepContextTokens` merges partial totals — absent fields leave the previously known values instead of coercing them to 0/undefined (the event type guard does not verify these fields). |

Both maps share one lifecycle: `clearStepContextFill(sessionId)` (a slightly misleading name kept so `handlePlanGenerated` can clear both with one action) drops the session's step fills AND token totals when a new plan is generated — plan step ids are reused across plans in a session, so a step whose fill has not been re-reported yet must not briefly show the previous plan's percentage. The maps survive session switches (A→B→A) like the rest of the session-keyed state. The same action also fires on session deletion (`performDelete` in `useSessionActions`), so neither map outlives its session — keeping the per-session maps bounded, consistent with `scrollPositions` and the other per-session slices. Project deletion cascades the same cleanup over the project's sessions via the bulk variant `dropSessions(sessionIds)` (one `set` for the whole batch, skipping absent ids so an all-unknown batch preserves state identity — same conventions as `chatInputStore`/`attachmentsStore`/`e2sStore`); `clearStepContextFill` and `dropSessions` alike are full no-ops (state object returned as-is) when the listed sessions have no entries in either map.
