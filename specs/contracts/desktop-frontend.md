...
| `GetSessionHistory`    | id, limit, before            | (\*session.HistoryPage, error) | One KEYSET page of message history (`{messages, next_cursor, has_more}`). `before` is the opaque cursor returned as `next_cursor` by the previous call ("" = newest page), so the UI pages older content in on scroll-up instead of loading the whole session; `limit`<=0 uses the backend default (200) and is capped (2000). Messages are ordered oldest-first within the page; non-content activity rows (`thinking`, `step_done`) are omitted |
| `GetSessionPlanTimeline` | id                        | ([]session.ChatMessage, error) | The session's plan-lifecycle rows — every `plan` declaration plus every `plan_step_start`/`plan_step_complete`/`plan_step_paused` row — in ascending stream order, independent of the chat window's pagination. Used after a history load whose newest page lost the declaration (a plan task appends thousands of rows after it) to rebuild the Execution Plan panel with full step statuses and re-insert the plan rows into the chat store; one indexed query over the plan roles, bounded by the handful of rows a declared plan produces (defensive `LIMIT` 5000) |
...
| `SetPaperPinned` | projectID, paperID, pinned | error | Pin/unpin a paper: records the card's research-root-relative document path (`papers/<slug>/paper.md`) in `ProjectInfo.ResearchPins.Papers`. Serializes on the per-research-root mutation mutex shared with the research pin RPCs and `Enable`/`DisableResearch`, merging only the pins delta under that lock. Pinning requires the card file to exist; unpinning tolerates a deleted card, so stale pins stay removable. Idempotent both ways; no event — the caller's resolved promise is its refresh signal |
