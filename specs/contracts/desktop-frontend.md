...
| `ConfigResponse`           | backend  | backend → frontend | Sanitized config view. Its per-provider entries expose the compatible-provider fields: `tls_fingerprint` (ADR-054 pin) and `auto_retry_seconds` (auto-resend interval, `omitempty` — absent/0 = disabled; the fixed `anthropic`/`chatgpt` entries always report 0 and never carry the key, [ADR-065](../decisions/065-auto-resend-retryable-errors.md)) |
...
| `LLMFullConfigRequest`    | frontend | frontend → backend | LLM multi-provider config update. Each `ProviderConfigRequest` entry carries the pointer-sentinel optional fields: `tls_fingerprint *string` (nil = keep persisted, non-nil `""` = clear — [ADR-054](../decisions/054-per-provider-tls-pinning.md)) and `auto_retry_seconds *int` (nil = keep persisted, non-nil applies verbatim so an explicit `0` disables the timer — [ADR-065](../decisions/065-auto-resend-retryable-errors.md)); both are ignored for the fixed `anthropic`/`chatgpt` providers, which have no such keys |
...
| `UpdateLLMConfig`       | LLMFullConfigRequest    | error                             | Update full LLM multi-provider config. Per-entry `auto_retry_seconds *int` (compatible providers only) uses the same pointer sentinel as `tls_fingerprint`: nil keeps the persisted interval (debounced partial saves must not silently reset or disable the auto-resend), non-nil applies verbatim, an explicit `0` disables ([ADR-065](../decisions/065-auto-resend-retryable-errors.md)); the fixed anthropic/chatgpt entries ignore the key |
