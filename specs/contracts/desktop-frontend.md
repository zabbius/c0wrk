...
| `PickAndImportTheme` | —     | (*ThemeDTO, error) | Native single-select theme picker that imports the chosen CSS in one action: a "Theme files" filter (`*.css`), cancel returns `(nil, nil)` (nothing imported, no error), a chosen path delegates to `ImportThemeFromPath`. Must remain on `App` — it requires the Wails context like `PickDirectory` |
