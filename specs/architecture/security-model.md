...
Plus a shared **misc-hardening** set (fork bombs, crontab/firewall flush, registry/scheduled-task tampering) and a **privilege-escalation** entry (`sudo`). The **SCM** entry blocks mutating git subcommands only — the authoritative list of blocked/unblocked subcommands lives in `backend/config/defaults.go`, locked by `TestApplyDefaults_GitMutatingBlacklist`; it is not duplicated here to avoid drift. Every git pattern anchors on a **command-position prefix** (`(^|[\s;&|(\x60]|[/\\])git` — start of input, after a statement/pipeline separator, an opening paren, or a backtick, or right after a path separator/backslash so path-qualified and escaped invocations like `/usr/bin/git rm x` and `./git push`, and `x\git push` are caught) followed by an optional **global-option preamble** (`gitGlobalOpts` — the `-c`/`-C` and `-p`/`-P` short switches plus value-taking long flags (`--git-dir`, `--work-tree`, `--config-env`, …) and standalone switches, where a flag's value is a shell word so quoted / space-containing and backslash-escaped values (`git -C "my repo" push`, `git -c "user.name=John Doe" commit -m x`) stay bound to their flag and a single global option cannot defeat every pattern at once) and then `\s+<subcommand>`, instead of `\b`: a bare `\b` also matched `git …` inside quoted strings that are DATA, not commands (`rg "git checkout"`, `git log --grep="git rebase"`), while command position still catches `$(git push)`, `cd repo && git push`, and `xargs git rm`. Design rationale: read-only commands (`status`, `log`, `diff`, `show`, `blame`, `ls-files`, `rev-parse`, `fetch`, …) are intentionally **not** blocked; dual-mode subcommands (`branch`, `tag`, `remote`, `stash`, `config`, `reflog`, `apply`, `clean`, `add`, `rm`, `submodule`, `worktree`, `notes`, `reset`, `sparse-checkout`) are matched by **inverted patterns that enumerate only their mutating spellings** (bare/index-changing forms like `git add`, `git rm`, `git stash`, `git config k v`, mutating flags like `reset --hard` / `config --unset`, and bare-name creates like `git branch x`, `git tag v1`), so their read-only spellings (`git branch -a`, `git tag -l`, `git config --get k`, `git stash list`, `git remote -v`, `git add -n`) flow through the ordinary group policy; unambiguous mutators (`commit`, `merge`, `rebase`, `push`, `clone`, …) are matched wholesale, alongside FN plugs for the transport/server faces (e.g. `send-pack`, `upload-pack`, `receive-pack`, `shell`, `http-backend`) that bypass the push/fetch porcelain, and a compensating `(?i)` interpreter wrapper for the quoting faces the command-position prefix alone would miss (`sh -c "git push"`, `bash -lc 'git push'`, `eval "git …"`, and the PowerShell/cmd faces on Windows); network/exfil-capable subcommands (`send-email`, `imap-send`, `daemon`, `instaweb`) are included; `git fetch` is excluded (additive / non-destructive — it only adds objects and updates remote-tracking refs). The posh git patterns carry a `(?i)` case-insensitive prefix and the `git(\.exe)?` spelling because PowerShell resolves the git executable case-insensitively (so `Git commit` / `GIT PUSH` still match); the bash patterns are deliberately case-sensitive since Unix executables are. Credential-file *reads* (e.g., `~/.ssh/id_rsa`) are intentionally left to the path-containment check rather than a literal-filename blacklist, since the same files live at unpredictable paths across systems.
