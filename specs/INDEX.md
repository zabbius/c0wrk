...
| SLM profiles (tuning for small/local models) | [domains/slm.md](domains/slm.md), [decisions/041-slm-profiles.md](decisions/041-slm-profiles.md), [decisions/022-small-llm-profile.md](decisions/022-small-llm-profile.md) |
...
- [slm.md](domains/slm.md) - SLM profiles: predefined (read-only) + custom (`~/.c0wrk/slm-profiles.yaml`) profile catalog resolved from `slm.enabled` + `slm.active_profile`; five variant sections (essential-tools narrowing, system-prompt Lite swap, sampling override, loop hardening, context management) for tuning c0wrk to small/local models
...
- [022-small-llm-profile.md](decisions/022-small-llm-profile.md) - Small-LLM profile: manual master toggle + five independently sub-toggled variants (essential tools, system-prompt Lite, sampling, loop hardening, context management) for tuning c0wrk to small/local models → Inline-knob config storage superseded by ADR-041; variant architecture stands
...
- [041-slm-profiles.md](decisions/041-slm-profiles.md) - SLM profile catalog: research-backed read-only predefined profiles ∪ operator custom profiles (`~/.c0wrk/slm-profiles.yaml`) replace the inline `small_llm.*` knobs; `config.yaml` persists only `slm.enabled` + `slm.active_profile` (generic soft fallback, sanctioned reset migration); five profile-scoped RPCs + the `SetSLMEnabled` master-toggle RPC (persisted toggle; enabling gate-checked, closing the experimental gate resets it) + suggested-profile hint (never auto-applied); `agent_metrics` carries profile identity; SLM nomenclature → Partially supersedes ADR-022
- [042-gpu-embedding-provider.md](decisions/042-gpu-embedding-provider.md) - GPU embedding execution provider: `execution_provider` auto/cpu/cuda with `device_id`, loud (never-silent) CPU fallback tracked at the sp4rk level, all ORT touches on a single locked thread, external `nvidia-smi` PID verification for GPU tests, cuda13-only opt-in packaging (`fetch-onnx-gpu`), flavor-aware version stamps
- [043-cuda-release-artifact.md](decisions/043-cuda-release-artifact.md) - CUDA release artifact: fifth release archive `c0wrk-desktop-linux-amd64-cuda13.tar.gz` (linux/amd64 only), always same-flavor in-app updates (flavor read from the install tree), flavor switches are manual reinstalls → Closes ADR-042's deferred packaging item, extends ADR-023's release matrix
