...
- [041-slm-profiles.md](decisions/041-slm-profiles.md) - Model Profiles catalog: research-backed read-only predefined profiles ∪ operator custom profiles (`~/.c0wrk/model-profiles.yaml`) replace the inline `small_llm.*` knobs; `config.yaml` persists only `model_profiles.enabled` + `model_profiles.active_profile` (generic soft fallback, sanctioned reset migration); five profile-scoped RPCs + the `SetModelProfilesEnabled` master-toggle RPC (persisted toggle; enabling gate-checked, closing the experimental gate resets it) + suggested-profile hint (never auto-applied); `agent_metrics` carries profile identity; Model Profiles nomenclature → Partially supersedes ADR-022 → The experimental-gate coupling is partially superseded by ADR-044
...
- [043-model-profiles-rename.md](decisions/043-model-profiles-rename.md) - Rename "Small LLM" / "SLM" to "Model Profiles": a deliberate clean break across config keys (`model_profiles.*`), store (`~/.c0wrk/model-profiles.yaml`), Go identifiers/packages, RPCs and UI labels — legacy `slm:` config and `slm-profiles.yaml` are ignored (sanctioned reset migration); historical ADRs keep their original prose
