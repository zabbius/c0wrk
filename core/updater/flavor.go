...
// flavors — but in the install tree: `make fetch-onnx-gpu` (ADR-045) drops
...
const (
	...
	// FlavorCUDA13 is the opt-in GPU flavor (ADR-045): a CUDA 13-flavored
	// ONNX Runtime with libonnxruntime_providers_cuda.so installed next to
	// the binary. One CUDA major is pinned deliberately (see ADR-045
