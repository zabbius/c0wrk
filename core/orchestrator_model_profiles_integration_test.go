...
// This file holds the ModelProfiles INTEGRATION tests. Unlike the per-variant unit
// tests (orchestrator_modelProfiles_test.go, systemprompt_test.go) that drive each
...
// buildSystemPrompt → applyModelProfilesToolFilter chain, driven by a real
...
// no-op (byte-identical prompt + untouched tool set vs. a zero-ModelProfiles
// baseline).
//
// The integration value over the unit tests is that the unit tests set ctx keys
// directly (WithModelProfilesLite / InjectionDefenseKey), bypassing the orchestrator
...
// TestModelProfiles_Integration_MasterOn_ComposesLitePromptFewShotInjectionDefenseAndReducedTools
...
// applyModelProfilesToolFilter must reduce the conductor's tool set.
func TestModelProfiles_Integration_MasterOn_ComposesLitePromptFewShotInjectionDefenseAndReducedTools(t *testing.T) {
	o := &Orchestrator{
	...
	if reqCtx.Value(ModelProfilesLiteKey) == nil {
		t.Fatal("master ON: prepareRequestContext did not set ModelProfilesLiteKey")
	}
...
// TestModelProfiles_Integration_MasterOff_ByteIdenticalPromptAndFullToolset is the
...
// from a zero-ModelProfiles config, and applyModelProfilesToolFilter returns the full
...
func TestModelProfiles_Integration_MasterOff_ByteIdenticalPromptAndFullToolset(t *testing.T) {
	...
	oOff := &Orchestrator{
		config: OrchestratorConfig{
			...
			ModelProfiles: ModelProfilesSettings{
				...
				SystemPrompt: ModelProfilesSystemPromptSettings{Lite: true},
			},
		},
		emitter: &noopEmitter{},
	}
	// Pre-profile baseline: a zero-ModelProfiles config.
	...
	if offCtx.Value(ModelProfilesLiteKey) != nil {
		t.Fatal("master OFF: prepareRequestContext set ModelProfilesLiteKey — defense-in-depth broken")
	}
	// Injection defense is independent of ModelProfiles, so it is set in both.
	if offCtx.Value(InjectionDefenseKey) == nil {
		t.Fatal("master OFF: InjectionDefenseKey should still be set (independent of ModelProfiles)")
	}
	...
	if offPrompt != baselinePrompt {
		t.Fatalf(
			"master OFF prompt diverges from zero-ModelProfiles baseline: "+
	...
	in := modelProfilesTestTools()
	got := oOff.applyModelProfilesToolFilter(in)
	if len(got) != len(in) {
