...
- **Self-hosted and corporate-friendly.** HTTP/HTTPS proxy support with custom CA certificates and bypass lists for outbound LLM, web, MCP, model-registry, and update traffic. An optional, experimental Model Profiles feature narrows the tool set, swaps in a compact system prompt, tightens sampling, and compacts context earlier for local or mid-size models.
