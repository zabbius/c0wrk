...
- Runtime config lives at `~/.c0wrk/config.yaml` (default dir constant `config.DefaultAgentDir = ".c0wrk"`). `config.example.yaml` is the authoritative reference for every tunable (LLM providers, executor loop caps, compaction thresholds, tool limits, timeouts, security policies, SLM profiles, vector index, experimental features, updates).
- **Experimental gate** (`experimental.enabled`, default false) is all-or-nothing. It gates the SLM profile and the E2S execution mode; disabling it clears the persisted `slm.enabled` (the Small-LLM master toggle — so re-enabling the gate never silently reactivates the profile), makes the profile ineffective, and hides its settings tab. RESEARCH is always available and is not gated by this switch. `backend/frontend_api_research.go` owns RESEARCH activation, workspace containment, non-destructive skill seeding, and watcher setup.
- **SLM profiles** (`slm.*`): tuning for small/local models via a profile catalog. Five read-only **predefined** profiles are compiled into the app (`qwen3.8-27b`, `qwen3.6-35b-a3b`, `gemma-4-26b-a4b-it`, `gemma-4-31b-it`, and the model-agnostic `generic`); operator **custom** profiles live in `~/.c0wrk/slm-profiles.yaml` and are always born as a duplicate of a catalog profile, then freely edited (stable slug ids, atomic rewrite, fail-soft load with visible warnings). `config.yaml` persists only two keys: the manual-only master toggle `slm.enabled` (default off; normally flipped from the Settings → Small LLM master switch via `SetSLMEnabled`, which persists it, fails closed on enable while the experimental gate is off, and is reset to false when that gate is closed) and `slm.active_profile` (default `generic`; a dangling id soft-falls back to `generic` with a load warning; a legacy inline `small_llm:` section is ignored at load and dropped by the next save — knob values do not migrate). Each profile carries the 25 knobs across five variant sections — tool-set narrowing (`essential_tools`), system-prompt Lite swap (`system_prompt`), sampling override (`sampling`), loop-hardening (`loop_hardening`), context-management overrides (`context`); every variant is gated by BOTH `experimental.enabled`, the master `slm.enabled`, and its own sub-toggle in the active profile's values (the `system_prompt` variant is the exception — it carries no separate sub-toggle and engages through `lite`). Profile CRUD at runtime via `GetSLMProfiles`/`CreateSLMProfile`/`UpdateSLMProfile`/`DeleteSLMProfile`/`SelectSLMProfile` plus the master-toggle `SetSLMEnabled` (predefined are read-only — duplicate to edit; deleting the active custom profile switches to `generic` with a one-shot notice); `GetSLMProfiles` reports the stored `slm.enabled` as its `enabled` field; `GetSLMProfiles` also returns a `suggested_profile_id` hint from the default model (never auto-applied) and every session's `agent_metrics` carries the active profile identity (`slm.profile` id-slug + `slm.profile_kind`). See `specs/domains/slm.md` and `specs/decisions/041-slm-profiles.md`.
