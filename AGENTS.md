...
- **Experimental gate** (`experimental.enabled`, default false) is all-or-nothing. It gates the E2S execution mode. RESEARCH is always available and is not gated by this switch. `backend/frontend_api_research.go` owns RESEARCH activation, workspace containment, non-destructive skill seeding, and watcher setup.
