...
- Runtime config lives at `~/.c0wrk/config.yaml` (default dir constant `config.DefaultAgentDir = ".c0wrk"`). `config.example.yaml` is the authoritative reference for every tunable (LLM providers, executor loop caps, compaction thresholds, tool limits, timeouts, security policies, model profiles, vector index, experimental features, updates).
- **Experimental gate** (`experimental.enabled`, default false) is all-or-nothing. It gates the E2S execution mode. RESEARCH is not gated by this switch — it is always on for every real project (there is no per-project toggle, and no persisted research root). `backend/frontend_api_research.go` owns the always-on RESEARCH status/graph RPCs, workspace containment (the root is unconditionally `<workspace>/.research`, `config.ProjectResearchPath`), the startup global pack seed (`seedGlobalPacks`, once per launch into `~/.c0wrk/.agents/{skills,agents}` BEFORE the skill/agent watchers start), and watcher setup. Skill/agent discovery precedence is project-local `<workspace>/.agents/*` → c0wrk global `~/.c0wrk/.agents/*` → user `~/.agents/*` (the `skills.dirs`/`agents.dirs` defaults, highest first) — the c0wrk global directory outranks `~/.agents`, so a seeded c0wrk pack (the `research-*` skills, `study-paper`, the research Subagent Profile) wins over a stale same-named user copy. The former per-project toggle, the `EnableResearch`/`DisableResearch` RPCs, and the project-local pack reconciliation were removed.
- **model profiles** (`model_profiles.*`): tuning for local / mid-size models via a profile catalog. Five read-only **predefined** profiles are compiled into the app (`qwen3.8-27b`, `qwen3.6-35b-a3b`, `gemma-4-26b-a4b-it`, `gemma-4-31b-it`, and the model-agnostic `generic`); operator **custom** profiles live in `~/.c0wrk/model-profiles.yaml` and are always born as a duplicate of a catalog profile, then freely edited (stable slug ids, atomic rewrite, fail-soft load with visible warnings). `config.yaml` persists only two keys: the manual-only master toggle `model_profiles.enabled` (default off; normally flipped from the Settings → Model Profiles master switch via `SetModelProfilesEnabled`, which persists it) and `model_profiles.active_profile` (default `generic`; a dangling id soft-falls back to `generic` with a load warning; a legacy inline `small_llm:` section is ignored at load and dropped by the next save — knob values do not migrate). Each profile carries the 25 knobs across five variant sections — tool-set narrowing (`essential_tools`), system-prompt Lite swap (`system_prompt`), sampling override (`sampling`), loop-hardening (`loop_hardening`), context-management overrides (`context`); every variant is gated by the master `model_profiles.enabled` and its own sub-toggle in the active profile's values (the `system_prompt` variant is the exception — it carries no separate sub-toggle and engages through `lite`). Profile CRUD at runtime via `GetModelProfiles`/`CreateModelProfile`/`UpdateModelProfile`/`DeleteModelProfile`/`SelectModelProfile` plus the master-toggle `SetModelProfilesEnabled` (predefined are read-only — duplicate to edit; deleting the active custom profile switches to `generic` with a one-shot notice); `GetModelProfiles` reports the stored `model_profiles.enabled` as its `enabled` field; `GetModelProfiles` also returns a `suggested_profile_id` hint from the default model (never auto-applied) and every session's `agent_metrics` carries the active profile identity (`model_profiles.profile` id-slug + `model_profiles.profile_kind`). See `specs/domains/model-profiles.md`, `specs/decisions/041-slm-profiles.md` and `specs/decisions/043-model-profiles-rename.md`.
- **E2S mode** (`e2s.*`, experimental, default off): a per-message alternative execution loop where the model's only memory is an externalized state Σ it patches every turn (`e2s_step` meta-tool: `state_patch` + `action`) — every turn is a fresh one-shot `[system, user]` request, so context stays O(1) in turns. Own loop in `core/e2s` on raw sp4rk primitives (no Executor/Conductor); actions dispatch through `ToolRegistry.Execute` with every security gate intact; hybrid schema (8 fixed core keys, mutable extensions with null-tombstone, byte cap, schema fingerprint); the Available Tools catalog carries full input schemas and action args are structurally validated pre-dispatch; observations truncate cache-on-truncate with a hash nudge (recover via `tool_result_read`); `batch` executes its sub-calls through the loop; `[turn N of M]` budget header with a wrap-up warning; semantic anti-spin (tool + target anchor plus the write/edit content payloads, ignores line ranges); per-step Σ persistence (`task_e2s_state`) + resume — step-limit exhaustion is a resumable checkpoint (fresh budget, Σ preserved); delegation via the injected conductorLauncher with an inert plan state; frontend Execution State panel replaces the plan view (`e2s_state` event: run-local `turn` + cumulative `total_turns`). Mutually exclusive with goal mode. See `specs/domains/e2s.md`, `specs/decisions/039-e2s-explicit-execution-state.md` and `specs/decisions/040-e2s-stabilization.md`.
- **Verify-on-edit** (`executor.verify_on_edit`, default off) runs a config-authored command after each successful `write_file`/`edit_file` in CODE tasks and injects bounded output as a system observation. It is disabled for CHAT and goal turns. The command skips interactive confirmation because it is model-inaccessible config, but `ExecuteUnattended` still enforces required fields, disabled tools, execute-group `deny`, the shell blocklist, **canonical** hard safety reasons (judge + symlink detection; the flowsh ⊤ limitation `command_unbounded_analysis` is deliberately non-canonical and passes — the command is the user's own config), and the `timeouts.bashMaxTimeout` ceiling. Never expose `ExecuteUnattended` to a model-facing path. See `specs/domains/verify-on-edit.md`.
- **Git auto-fetch** (`git.auto_fetch`, default on; `git.auto_fetch_interval`, default "2m") keeps the active CODE project's remote-tracking refs fresh via background `git fetch` on project switch (app startup rides along), a periodic ticker, and window focus — one server-side gated funnel with a 60s shared min-interval, a TryLock-skip past manual remote ops, Debug-only failures, and a hardened spawn carrying `GIT_TERMINAL_PROMPT=0`. Interval `"0"` disables only the ticker; manual Git-panel fetch is never gated by this section. See `specs/domains/git-auto-fetch.md`.
...
- **Security/tool policies** are enforced in `core/builder.go` → `applySecurityPolicies` from `config.Security.Groups` (`security.groups.<group>.{policy, blocklist?}` — per-capability-group policies, ADR-024; short enum `allow`/`user_confirm`/`deny`; mutating groups default to `user_confirm`; blocklist patterns must compile and are execute-only; the blocklist is a user extension that is **empty by default** — c0wrk ships no predefined patterns, the deterministic shell floor is the flowsh criteria C1–C10, ADR-052/ADR-057/ADR-061). Pending confirmations flow through `App.pendingConfirmations` sync.Map back to the UI.
...
| `reviewStore`        | Review page / loop state (per-session comments, review-loop reopen, diff view mode)           |
...
| `e2sStore`           | Per-session E2S execution-state Σ snapshots from `e2s_state` events (the backend owns the merge — the store keeps the latest full Σ and replaces it outright; cleared on session switch/delete) |
...
- `groupMessages()` transforms flat `ChatMessageUI[]` into a `DisplayItem[]` tree (21 kinds: user, assistant, thought, thought_group, tool, tool_confirm, ask_user, step_limit, autonomy_decision, resume_action, error, service, plan_step, subagent, reflection, step_finish, context_compaction, memory_read, plan_review, goal_proposal, checklist).
...
- **Execution Panels**: Collapsible plan view with DAG graph (SVG, lane allocation) + item list, click-to-scroll to chat. E2S sessions render the **Execution State panel** (`ExecutionStatePanel`) *instead of* the plan view: the Σ checklist with checked states, the core fields (objective/status/files_touched/findings/decisions/done_criteria/next_steps), a run-local `turn N/M` counter (bare `turn N` when the run is unbudgeted), and a status badge — driven by full `e2s_state` snapshots in `e2sStore`; flips back to the plan view on session switch/delete or when a fresh non-E2S task starts.
...
- **Research Panel**: Workspace-contained hypothesis graph/metrics view, always on for a real project (no toggle); in No-Project the panel renders nothing and issues no RPC. It refreshes from lightweight `research:*` events.
