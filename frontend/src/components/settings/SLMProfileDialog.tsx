...
export type SLMProfileDialogKind = 'duplicate' | 'rename' | 'delete'

interface SLMProfileDialogProps {
  kind: SLMProfileDialogKind
...
const NAME_LABELS: Record<Exclude<SLMProfileDialogKind, 'delete'>, { title: string; action: string }> = {
 ...
 * units.
 */
export function SLMProfileDialog({
  kind,
  profileName,
  defaultName = '',
  busy = false,
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md" data-testid={`slm-profile-dialog-${kind}`}>
        {kind === 'delete' ? (
          <>
            <DialogHeader>
          ...
          <>
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="slm-profile-name" className="text-xs text-muted-foreground">
                Profile name
              </label>
              <Input
                id="slm-profile-name"
                ...
                data-testid="slm-profile-name-input"
              />
            </div>
            {error && (
              <div className="flex items-start gap-2 p-2 rounded-md bg-destructive/10 border border-destructive/20 text-xs">
