...
import { SLMProfileSelector } from './SLMProfileSelector'
import { SLMProfileDialog, type SLMProfileDialogKind } from './SLMProfileDialog'
import {
...
import { ContextSection } from './SLMContextSection'
import type {
  SLMProfilesResponse,
  SLMProfileValues,
  SLMEssentialTools,
...
/** Frontend mirror of the backend generic fallback id (config.SLMGenericProfileID). */
const GENERIC_PROFILE_ID = 'generic'

/**
 * Small-LLM settings tab. A master toggle (config.yaml slm.enabled) sits at the
 ...
 * SetSLMEnabled and reveals the profile picker above the five variant sections.
 ...
 */
export function SLMSettings() {
  const [resp, setResp] = useState<SLMProfilesResponse | null>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [busy, setBusy] = useState(false)
  /** Dismissed suggestion id: stays hidden until the suggestion (model) changes. */
  const [dismissedSuggestion, setDismissedSuggestion] = useState<string | null>(null)
  const [dialog, setDialog] = useState<SLMProfileDialogKind | null>(null)
  ...
  useEffect(() => {
    getSLMProfiles()
      ...
      .catch((err) => logger.error('Failed to load Small LLM profiles:', err))
      .finally(() => setIsLoading(false))
  }, [])

  const reload = useCallback(async () => {
    const r = await getSLMProfiles()
    setResp(r)
    setError(null)
  }, [])
  ...
  const save = useCallback(
    async (next: SLMProfileValues) => {
      ...
      try {
        await updateSLMProfile(active.id, { config: next })
        setError(null)
      } catch (err) {
        logger.error('Failed to update Small LLM profile:', err)
        setError(err instanceof Error ? err.message : String(err))
        try {
          setResp(await getSLMProfiles())
        } catch {
  ...
  const patchValues = (mutate: (v: SLMProfileValues) => SLMProfileValues) => {
  ...
  const runProfileOp = async (op: () => Promise<void>) => {
    ...
    try {
      ...
      logger.error('Small LLM profile operation failed:', err)
      setError(err instanceof Error ? err.message : String(err))
    } finally {
      setBusy(false)
    }
  ...
  /**
   * Master switch (config.yaml slm.enabled): persist + apply, then reload the
  ...
  const handleToggleEnabled = async (next: boolean) => {
    ...
    setBusy(true)
    try {
      await setSLMEnabled(next)
      await reload()
    } catch (err) {
      logger.error('Failed to toggle Small LLM enabled:', err)
      setError(err instanceof Error ? err.message : String(err))
    } finally {
      setBusy(false)
    }
  ...
  const handleSelect = (id: string) => {
    ...
    void runProfileOp(() => selectSLMProfile(id))
  }

  const openDialog = (kind: SLMProfileDialogKind) => {
    setDialogError(null)
  ...
  const handleDialogConfirm = async (name: string) => {
    ...
    try {
      if (dialog === 'duplicate') {
        ...
        const newId = await createSLMProfile(effective.id, name)
        await selectSLMProfile(newId)
      } else if (dialog === 'rename' && active) {
        await updateSLMProfile(active.id, { name })
      } else if (dialog === 'delete' && active) {
        await deleteSLMProfile(active.id)
      }
      await reload()
      setDialog(null)
    } catch (err) {
      logger.error('Small LLM profile dialog operation failed:', err)
  ...
  // Master toggle (config.yaml slm.enabled): the first element, always visible.
  ...
  const masterToggle = (
    <div className="flex flex-col gap-2 p-4 rounded-lg border border-border bg-card/50">
      <Toggle
        ...
        disabled={busy}
        label="Enable the Small-LLM profile"
        description="Apply the selected profile's tool, prompt, sampling, loop and context overrides to Small-LLM runs. When off, none of the profile settings below take effect."
      />
      {busy && (
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <p className="text-xs text-muted-foreground">
          The Small-LLM profile is off. Turn it on to configure and apply a profile.
        </p>
        {error && (
  ...
  const essentialView: SLMEssentialTools = {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 p-4 rounded-lg border border-border bg-card/50">
        ...
        {suggestionVisible && (
          <div
            data-testid="slm-suggestion-banner"
            role="status"
      ...
      <EssentialToolsSection
        slice={essentialView}
        patch={(p: Partial<SLMEssentialTools>) =>
      ...
      <SystemPromptSection
        ...
        patch={(p: Partial<SLMSystemPrompt>) =>
      ...
      <SamplingSection
        ...
        patch={(p: Partial<SLMSampling>) => patchValues((v) => ({ ...v, sampling: { ...v.sampling, ...p } }))}
      ...
      <LoopHardeningSection
        ...
        patch={(p: Partial<SLMLoopHardening>) =>
      ...
      <ContextSection
        ...
        patch={(p: Partial<SLMContext>) => patchValues((v) => ({ ...v, context: { ...v.context, ...p } }))}
